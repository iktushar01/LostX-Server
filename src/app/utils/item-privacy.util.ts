...
export const applyPublicItemPrivacy = <T extends PublicItem>(
...
): T & { locationHidden?: boolean; imageBlurred?: boolean; imageHidden?: boolean } => {
    const canSeeDetails =
        context.isOwner ||
    ...
    const hideDescription =
        item.showDescriptionPublic === false ||
        (sensitive && item.showDescriptionPublic !== true);

    if (hideDescription) {
        result.description = "Details hidden for privacy — view after verification.";
    } else if (
        item.category === ItemCategory.ID_CARD ||
        item.category === ItemCategory.WALLET ||
        item.category === ItemCategory.PHONE
    ) {
        result.description = "Details hidden for privacy — view after verification.";
    }

    const hideExactLocation = item.showLocationPublic !== true;
    if (hideExactLocation) {
        result.location = buildPublicAreaLabel(item);
        result.locationHidden = true;
    }

    const hideImage = item.showImagePublic === false;
    if (hideImage) {
        result.imageUrl = null;
        result.imageHidden = true;
    } else if (sensitive && item.imageUrl) {
...
export const applyFoundItemLocationPrivacy = <T extends PublicItem>(
    ...
    const withVisibility = applyPublicItemPrivacy(item, context);

    if (item.showLocationPublic === true) {
